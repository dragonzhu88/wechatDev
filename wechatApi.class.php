...
class wechatCallbackApi
{
    ...
    private function receiveText($object)
    {
        ...
        if (strstr($keyword, "文本")){
            $content = "这是个文本消息";
        }else if(strstr($keyword,'消费')){
            $this->transmitTemplateMsg();
        }
        else if(strstr($keyword,'游戏')){
            $content = "回复:2048 玩2048游戏";
        }
        else if(strstr($keyword,'2048')){
            $content = $this->game2048();
        }
        else if(strstr($keyword,'成绩')){
            $content = $this->checkscore();
        }
            ...
            // $content = $this->wheather_check($keyword);
    ...
    private function transmitTemplateMsg(){
        require_once('wechatBase.class.php');
        $weixin = new WechatBase();
    ...
    private function call_robot($openid,$data){
        require_once('wechatBase.class.php');
        $weixin = new WechatBase();
        $content = $weixin->robot_ask($openid,$data);
        return $content;
    }

    private function wheather_check($data){
        require_once('wechatBase.class.php');
        $weixin = new WechatBase();
        $content = $weixin->wheather_check($data);
        return $content;
    }
    ...
    private function game2048(){
        $content = array();
        $content[] = array("Title"=>"2048游戏",
